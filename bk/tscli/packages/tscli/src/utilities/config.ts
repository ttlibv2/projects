...
import { PackageManager } from '../../lib/config/PackageManager';
import { globalFileName as globalFile, monorepoCfgFileName } from './constant';
...
const configNames = monorepoCfgFileName;//['angular.json', '.angular.json'];
const globalFileName = globalFile;
...
export class NgxWorkspace {
  ...
  static async load(workspaceFilePath: string): Promise<NgxWorkspace> {
...
const cachedWorkspaces = new Map<string, NgxWorkspace | undefined>();

export async function getWorkspace(level: 'global'): Promise<NgxWorkspace>;
export async function getWorkspace(level: 'local'): Promise<NgxWorkspace | undefined>;
export async function getWorkspace(level: 'local' | 'global'): Promise<NgxWorkspace | undefined>;
export async function getWorkspace(level: 'local' | 'global'): Promise<NgxWorkspace | undefined> {
  ...
  if (!configPath) {
    if (level === 'global') {
      ...
      const globalWorkspace = new NgxWorkspace(
  ...
  try {
    const workspace = await NgxWorkspace.load(configPath);
...
function findProjectByPath(workspace: NgxWorkspace, location: string): string | null {
...
export function getProjectByCwd(workspace: NgxWorkspace): string | null {
