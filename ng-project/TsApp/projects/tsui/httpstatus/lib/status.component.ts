...
import {PropCls, StyleCls, ImageObject, ButtonProps} from "ts-ui/common";
...
@Component({
  ...
  imports: [CommonModule, ImageModule, Button, ButtonDirective],
...
export class StatusPageView implements OnChanges, OnInit {
  ...
  onClickHomeBtn(event: any) {
    if(this.homeUrl && this.router) {
      const url = isString(this.homeUrl) ? [this.homeUrl] : this.homeUrl;
