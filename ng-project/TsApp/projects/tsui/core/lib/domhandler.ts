...
export class DomHandler {
    ...
    public static removeMultipleClasses(element, classNames) {
        if (element && classNames) {
            [classNames]
                .flat()
                .filter(Boolean)
                .forEach((cNames) => cNames.split(' ').forEach((className) => DomHandler.removeClass(element, className)));
        }
    }
    ...
    public static findSingle(element: any, selector: string): any {
        return DomHandler.isElement(element) ? element.querySelector(selector) : null;
    }
    ...
    public static appendOverlay(overlay: any, target: any, appendTo: any = 'self') {
        if (appendTo !== 'self' && overlay && target) {
            DomHandler.appendChild(overlay, target);
        }
    }
    ...
    public static relativePosition(element: any, target: any, gutter: boolean = true): void {
        ...
        gutter && (element.style.marginTop = DomHandler.transformOriginIsBottom(element) ? 'calc(var(--p-anchor-gutter) * -1)' : 'calc(var(--p-anchor-gutter))');
    }

    public static transformOriginIsBottom(element: any): boolean {
        return (element?.style?.transformOrigin ?? '').includes(`bottom`);
    }
    ...
    static getParents(element: any, parents: any = []): any {
        return element['parentNode'] === null ? parents : DomHandler.getParents(element.parentNode, parents.concat([element.parentNode]));
    }
    ...
    static getScrollableParents(element: any) {
        ...
        if (element) {
            let parents = DomHandler.getParents(element);
    ...
    public static calculateScrollbarWidth(el?: HTMLElement): number {
        if (el) {
            ...
            if (DomHandler.calculatedScrollbarWidth !== null) return DomHandler.calculatedScrollbarWidth;
    ...
    public static calculateScrollbarHeight(): number {
        if (DomHandler.calculatedScrollbarHeight !== null) return DomHandler.calculatedScrollbarHeight;
        ...
        DomHandler.calculatedScrollbarWidth = scrollbarHeight;
    ...
    public static getBrowser() {
        if (!DomHandler.browser) {
            let matched = DomHandler.resolveUserAgent();
            DomHandler.browser = {};
    ...
    public static getFocusableElements(element, selector = ''): any[] {
        let focusableElements = DomHandler.find(element, DomHandler.getFocusableSelectorString(selector));

        let visibleFocusableElements = [];

        for (let focusableElement of focusableElements) {
            const computedStyle = getComputedStyle(focusableElement);
            if (DomHandler.isVisible(focusableElement) && computedStyle.display != 'none' && computedStyle.visibility != 'hidden') visibleFocusableElements.push(focusableElement);
        }
    ...
    public static getFocusableElement(element, selector = ''): any | null {
        let focusableElement = DomHandler.findSingle(element, DomHandler.getFocusableSelectorString(selector));

        if (focusableElement) {
            const computedStyle = getComputedStyle(focusableElement);
            if (DomHandler.isVisible(focusableElement) && computedStyle.display != 'none' && computedStyle.visibility != 'hidden') return focusableElement;
        }
    ...
    static generateZIndex() {
        DomHandler.zindex = DomHandler.zindex || 999;
        return ++DomHandler.zindex;
    }
        ...
        switch (target) {
            ...
            default:
                ...
                if (type === 'string') {
                    ...
                    return DomHandler.isExist(target.nativeElement) ? target.nativeElement : undefined;
                }

                const isFunction = (obj: any) => !!(obj && obj.constructor && obj.call && obj.apply);
                const element = isFunction(target) ? target() : target;

                return (element && element.nodeType === 9) || DomHandler.isExist(element) ? element : null;
        }
    }
    ...
    public static blockBodyScroll(className = 'p-overflow-hidden') {
        document.body.style.setProperty('--scrollbar-width', DomHandler.calculateBodyScrollbarWidth() + 'px');
    ...
    public static createElement(type, attributes = {}, ...children) {
        if (type) {
            ...
            DomHandler.setAttributes(element, attributes);
    ...
    public static setAttribute(element, attribute = '', value) {
        if (DomHandler.isElement(element) && value !== null && value !== undefined) {
            element.setAttribute(attribute, value);
        }
    }

    public static setAttributes(element, attributes = {}) {
        if (DomHandler.isElement(element)) {
    ...
    public static isFocusableElement(element, selector = '') {
        return DomHandler.isElement(element)
