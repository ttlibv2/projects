...
function parseJsonSchemaToOptions(registry_1, schema_1) {
    return __awaiter(this, arguments, void 0, function* (registry, schema, interactive = true) {
        const options = [];
        function visitor(current, pointer, parentSchema) {
            ...
            const option = Object.assign({ name, description: '' + (current.description === undefined ? '' : current.description), default: defaultValue, choices: enumValues.length ? enumValues : undefined, required,
                alias,
                format,
                hidden,
                userAnalytics,
                deprecated,
                positional }, (types[0] === 'object'
                ? {
                    type: 'array',
...
function addSchemaOptionsToCommand(localYargs, options, includeDefaultValues) {
    ...
    for (const option of options) {
        ...
        const sharedOptions = Object.assign({ alias,
            hidden,
            description,
            deprecated,
            choices, coerce: itemValueType ? coerceToStringMap.bind(null, dashedName) : undefined }, (includeDefaultValues ? { default: defaultVal } : {}));
        if (positional === undefined) {
            localYargs = localYargs.option(dashedName, Object.assign({ array: itemValueType ? true : undefined, type: itemValueType !== null && itemValueType !== void 0 ? itemValueType : type }, sharedOptions));
        }
        else {
            localYargs = localYargs.positional(dashedName, Object.assign({ type: type === 'array' || type === 'count' ? 'string' : type }, sharedOptions));
        }
