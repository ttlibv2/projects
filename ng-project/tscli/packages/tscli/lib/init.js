...
(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (environment_options_1.disableVersionCheck || rawCommandName === 'new') {
        return (yield Promise.resolve().then(() => require('./cli'))).default;
    }
    let cli;
    try {
        ...
        const projectLocalCli = cwdRequire.resolve('@tsui/cli');
        cli = yield Promise.resolve(`${projectLocalCli}`).then(s => require(s));
        ...
        if (!localVersion) {
            try {
                const localPackageJson = yield node_fs_1.promises.readFile(path.join(path.dirname(projectLocalCli), '../../package.json'), 'utf-8');
        ...
        if (isGlobalGreater &&
            ...
            if (rawCommandName === 'update' &&
                ...
                cli = yield Promise.resolve().then(() => require('./cli'));
            }
            else if (yield (0, config_1.isWarningEnabled)('versionMismatch')) {
