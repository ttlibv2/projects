...
import { NgxWorkspace, getWorkspace } from '../utilities/config';
...
export async function runCommand(args: string[], logger: logging.Logger): Promise<number> {
  ...
  const { $0, _, help = false, jsonHelp = false, getYargsCompletions = false, ...rest} = sparse;
  ...
  let workspace: NgxWorkspace | undefined;
  let globalConfiguration: NgxWorkspace;
  try {
    [workspace, globalConfiguration] = await Promise.all([getWorkspace('local'),getWorkspace('global'),]);
  } catch (e) {
    assertIsError(e);
