...
function jsonHelpUsage() {
    ...
    const subcommands = usageInstance.getCommands()
        .map(([name, rawDescription, isDefault, aliases, deprecated]) => (Object.assign(Object.assign({ name: name.split(' ', 1)[0].replace(yargsDefaultCommandRegExp, ''), command: name.replace(yargsDefaultCommandRegExp, ''), default: isDefault || undefined }, parseDescription(rawDescription)), { aliases,
        deprecated })))
    ...
    const output = Object.assign(Object.assign({ name: [...context.commands].pop(), command: `${command === null || command === void 0 ? void 0 : command.replace(yargsDefaultCommandRegExp, localYargs['$0'])}${defaultSubCommand}` }, parseDescription(rawDescription)), { options: normalizeOptions.sort((a, b) => a.name.localeCompare(b.name)), subcommands: otherSubcommands.length ? otherSubcommands : undefined });
