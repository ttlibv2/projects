...
import { NgxWorkspace } from '../utilities/config';
...
export interface CommandContext {
  ...
  root: string;
  workspace?: NgxWorkspace;
  globalConfiguration: NgxWorkspace;
...
export abstract class CommandModule<T extends {} = {}> implements CommandModuleImplementation<T> {
  ...
  protected async getAnalytics(): Promise<any> {
  ...
  protected addSchemaOptionsToCommand<T>(localYargs: Argv<T>, options: Option[]): Argv<T> {
    const optionsWithAnalytics = addSchemaOptionsToCommand(localYargs,options,this.context.args.options.help);
  ...
  protected getWorkspaceOrThrow(): NgxWorkspace {
...
//   ): Partial<Record<EventCustomDimension | EventCustomMetric, string | boolean | number>> {
    ...
    const parameters: Partial<Record<any, any>> = {};
