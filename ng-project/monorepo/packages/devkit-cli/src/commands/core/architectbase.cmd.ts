  ...
  implements ICommandModule<T>
{
  ...
  protected getArchitectHost(): WorkspaceNodeModulesArchitectHost {
    ...
    // const workspace: AngularWorkspace = this.getWorkspaceOrThrow();

    // return this._architectHost = new WorkspaceNodeModulesArchitectHost(<any>workspace, workspace.basePath);
    throw new Error(`architectbase.cmd.ts::getArchitectHost`);
  }
  ...
  private warnOnMissingNodeModules(): void {
    const basePath = this.context.workspace?.baseDir;
    if (!basePath) {
      return;
    }
