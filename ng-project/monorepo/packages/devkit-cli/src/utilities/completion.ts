...
// Should you change your mind, the following command will set up autocompletion for you:
//
//     ${colors.yellow(`ng completion`)}  `.trim(),);
//
...
//         ' is typically done with the `-g` flag in `npm install -g @angular/lib`.' +
//         '\n\n' +
...
//   const {cli} = await DevWorkspace.global();
//   return cli?.extensions?.['completion'] as CompletionConfig;
// }
//
// async function setCompletionConfig(config: CompletionConfig): Promise<void> {
//   const ws = await DevWorkspace.global();
//   if (!ws) throw new Error(`Could not find global workspace`);
//
//   const cli: any = ws.cli ?? null;
//   if (!json.isJsonObject(cli as JsonObject)) {
//     throw new Error(
//       `Invalid config found at ${ws.filePath}. \`cli\` should be an object.`,
//     );
//   }
//   cli.extensions['completion'] = config as json.JsonObject;
//   await ws.write();
// }
//
