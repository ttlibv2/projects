...
export function getLocalPackageUrl(packageName: string) {
  ...
  const globalNodeModulesPath: string = getGlobalNodeModulesPath(packageName); // Implement this based on the environment
...
function getGlobalNodeModulesPath(packageName: string) {
  try {
    ...
    throw new Error(`Module ${packageName} not found globally.`);
    //return null;
  }
}
